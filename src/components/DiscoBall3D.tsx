...
function DiscoBall3D({ rotationSpeed, isMaxSpeed }: DiscoBall3DProps) {
  ...
  // Рассчитываем размер плитки с учётом швов между ними
  // Размер плитки = окружность на экваторе / количество плиток * коэффициент для швов
  const tileSize = (2 * Math.PI * 2.5) / 38 * 0.96 // 0.96 создаёт тёмные швы между плитками

  return (
    <group ref={groupRef}>
      {/* Основная тёмная сфера-основа (швы будут видны как тёмные промежутки) */}
      <mesh>
